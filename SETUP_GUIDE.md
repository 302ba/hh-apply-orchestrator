...
- `LOG_FILE` — файл журнала, по умолчанию `logs/auto-apply.log`
...
Вывод `npm run apply` сохраняется в `logs/auto-apply.log`. Авторизация через `npm run login` по-прежнему использует консоль, потому что ожидает ручной вход в браузере.
